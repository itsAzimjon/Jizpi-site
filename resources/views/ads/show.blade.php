...
<div class="container">
    <div class="row temple-news">
        ...
        <div class="col left col-12 col-sm-12 col-md-12 p-3 col-lg-8 col-xl-8 col-xxl-8 border-light-subtle border-end">
            ...
            <h4 class="my-4">{{ __($ad->title) }}</h4>
            <p>{!!nl2br (__($ad->description)) !!}</p>
        </div>
        <div class="col right col-lg-4 col-xl-4 col-xxl-4">
            ...
            @foreach ($ads->sortByDesc('created_at')->take(4) as $xad)
                @if ($xad->id != $ad->id)
                    <a href="{{ route('ads.show', ['ad' => $xad->id])}}">
                        <p class="add-date">
    ...
    @auth
    <div class="row">
        <form class="col-1 mx-2 d-flex justify-contend-between" action="{{ route('ads.destroy', $ad->id) }}" method="POST">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger" onclick="return confirm('{{ $ad->id}} O\'chirishni tasdiqlaysizmi?')">
        ...
        <a href="{{ route('ads.edit', ['ad' => $ad->id]) }}" class="btn btn-light btn-sm col-1" >
