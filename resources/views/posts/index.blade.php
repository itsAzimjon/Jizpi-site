...
<div class="container">
    <div class="row allNews">
        ...
        @foreach ($posts as $post)
            @php
                $currentMonth = \Carbon\Carbon::parse($post->created_at)->format('F Y');
            @endphp
            ...
            <div class="col-4">
                <div class="contNews p-2">
                    ...
                    <p class="fp">{{ __($post->category->name)}}</p>
                    <p class="sp mt-0">{{ __($post->title) }}</p>
                    <p class="tp">{{ \Carbon\Carbon::parse($post->created_at)->format('F j, Y')}}</p>
                    <a href="{{ route('posts.show', ['post' => $post->id])}}" class="lp">{{ __('O‘qishda davom eting') }} 
